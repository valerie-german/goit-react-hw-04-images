...
state = { imagesArray: null, loading: false, error: null, }; - записываем ошибку
в стейт

.then(response => { if (response.ok) { return response.json(); } return
Promise.reject(new Error(this.props.errorMessage())); }) - создаем ошибку, если
статус не ОК

        errorMessage() - приходит из пропса (Тоастер-нотификация)

.catch(error => { this.setState({ error: error }); - ловим ошибку и меняем стейт

Теперь можно зарендерить любую разметку

2.ПАТТЕРН STATE MACHINE МАШИНА СОСТОЯНИЙ Создаем 4 состояния

'idle'- простой (стоит на месте) - запроса ещё нет
...
Добавляем в стейт status и все рендеры по условию заменяем на рендер в
зависимости от значения статуса.

3. Разметку лучше абстрагировать в отделные компонеты
